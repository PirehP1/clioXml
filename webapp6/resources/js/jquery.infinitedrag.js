	...
	var InfiniteDrag = function(draggable, draggable_options, tile_options) {
		...
		var update_containment = function() {
			...
			var half_width = _to.width / 2,
				...
				viewport_offset = $viewport.offset(), //$viewport.offset(),
			...
			var Mx = (maxx) + viewport_offset.left + viewport_draggable_width;
			var mx = (minx) + viewport_offset.left;
			console.log(mx,Mx);
			console.log(containment); // recalculer correctement le containment car c'est lui qui est mauvais (notament le width)
			$draggable.draggable("option", "containment", containment);
			
		};
		
		$draggable.update_containment=update_containment;
